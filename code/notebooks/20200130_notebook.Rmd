...
• **Hernandez-Pacheco et al. (2017)** Identification of a novel locus associated with skin colour in African-admixed popuilations, *Scientific Reports*: <https://www.nature.com/articles/srep44548>. 9 hits with genome-wide significance here: `data/20170316_Hernandez-Pacheco-et-al.xlsx`.

Others compiled into the single XLSX doc `data/20200622_pigmentation_snps.xlsx`:
• '20190322_Jonnalagadda-et-al': **Jonnalagadda et al. (2019)** A Genome-Wide Association Study of Skin and Iris Pigmentation among Individuals of South Asian Ancestry, *Genome Biology and Evolution"
...
test <- lapply(seq(1:3), function(x){
  readxl::read_excel(here("data", "20200622_pigmentation_snps.xlsx"),
                     sheet = x)
})

sheet_names <- readxl::excel_sheets(here("data", "20200622_pigmentation_snps.xlsx"))
test <- lapply(sheet_names, function(x){
  readxl::read_excel(here("data", "20200622_pigmentation_snps.xlsx"),
                     sheet = x)
})
names(test) <- sheet_names
```
