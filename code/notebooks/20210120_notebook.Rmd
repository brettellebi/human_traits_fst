...
```{r, eval = F}
date = "20210122"
dest_dir = here::here("data", paste(date, "_snp_hit_lists", sep = ""))
...
lapply(data_list, function(pheno){
  ...
  # Set file basename
  trait = names(data_list)[counter]
  filename = paste(target_snps_pref, trait, "_with_P.txt", sep = "")
  # Write SNPs to file
  df %>% 
...
snps_pref=data/20210122_snps
out_pref=..vcfs/snphits
...
for trait in $(echo $traits ); do
  bsub \
    ...
    gatk SelectVariants \
      -R $ref \
      -V $in_vcf \
      --keep-ids $snps_pref\_$trait.list \
      -O $out_pref\_$trait.vcf.gz 
    """ ;
done
```
...
target_snps_pref=data/20210119_snps_
out_dir=data/20210120_plink2_alfreqs
...
for trait in $(echo $traits); do
  plink2 \
    --vcf $in_vcf \
    --extract $target_snps_pref\_$trait.list \
    --freq \
    --max-alleles 2 \
    --snps-only \
...
Create function for reading `Plink` `.afreq` files
