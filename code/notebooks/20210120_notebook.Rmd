...
```{r, eval = F}
# Save list
neg_cont_list = lapply(dat_lst, function(pheno){
  df = pheno[["consol"]]
  
  df = df %>% 
    dplyr::select(TOP_SNP, RISK_ALLELE, RISK_AF, BIN_100)
  
  return(df)
})

saveRDS(neg_cont_list, here::here("data", "20210121_neg_cont_list.rds"))
```
