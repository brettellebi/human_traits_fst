...
lapply(dat_lst, function(pheno){
  ...
  # Write SNPs to file
  readr::write_lines(pheno[["clean"]]$TOP_SNP, here("data", filename))
})

# SNP and P with header for `Plink`
counter <- 0
lapply(dat_lst, function(pheno){
  # Set counter
  counter <<- counter + 1
...
### Get allele frequencies per chromosome
...
target_popns = c("CEU", "CHS", "YRI")

counter <- 0
test = lapply(dat_lst, function(pheno){
  # set counter 
  counter <<- counter + 1
  # get trait name
  trait = names(dat_lst)[counter]
  # get paths to target files
  target_path = file.path(target_dir, trait)
  # read in data
  clumped_df = read.table(target_path, header = T)
  # add to list
  pheno[["clumped"]]  = clumped_df
...
trait = "hei"
  # get target paht
  target_path = file.path(target_dir, trait)
  # read in global AF file
  global_AF = read.table(file.path(target_path, paste(trait, "all.afreq", sep = ".")),
                         comment.char = "",
                         header = T) %>% 
    dplyr::mutate(POPN = "all")
  # read in popn files
  str_match = paste(target_popns, collapse = "|")
  popn_files = list.files(target_path, pattern = str_match, full.names = T)
  names(popn_files) = target_popns
  popn_AF = lapply(popn_files, function(popn_file){
    df = read.table(popn_file,
                    comment.char = "",
                    header = T)
  }) %>% dplyr::bind_rows(.id = "POPN")
  # bind `global_AF` and `popn_AF` and spread
  df = rbind(global_AF, popn_AF) %>% 
    tidyr::pivot_wider(id_cols = c(X.CHROM, ID, REF, ALT),
                       names_from = POPN,
                       values_from = ALT_FREQS)
  # read in data
...
```{r}
# Get list of files
target_files = list.files(here::here("data", "20210120_plink2_alfreqs"),
                          pattern = "all.afreq",
                          recursive = T,
                          full.names = T)
