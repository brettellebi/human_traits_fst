...
# For each bin in `risk_list`, pull out a random number of 1KG SNPs with the same bin
...
## Run over list
counter <- 0
test = lapply(risk_list[1:2], function(bin_df){
  # set counter 
  counter <<- counter + 1
  # get target bin
  ...
  hits_n = nrow(bin_df)
  # filter 1kg DF for SNPs with same bin and get random 
  random_hits = freq_df %>% 
