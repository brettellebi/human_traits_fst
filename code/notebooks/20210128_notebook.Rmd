...
```{r,fig.show="hold", out.width='50%', fig.cap = "Cases vs controls"}
one = fst_out %>%
...
```{r, warning = F}
ks_out = split(fst_out, f = fst_out$case_control)
...
#                 "P_LESS" = results_two_sided[["p.value"]],
...
#      return(df_out)
      ...
      dplyr::bind_rows(.id = "TRAIT_B")
  }) %>% 
    dplyr::bind_rows(.id = "TRAIT_A")
}) %>% 
  dplyr::bind_rows(.id = "CASE_CONTROL") %>% 
  dplyr::mutate(across(c("TRAIT_A", "TRAIT_B"),
                       ~factor(.x, levels = trait_levels_verb)))
