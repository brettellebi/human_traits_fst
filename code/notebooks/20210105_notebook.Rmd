...
```{r, engine='bash', eval=F}
# All
bsub -M 10000 -o log/plink.out -e log/plink.err \
  "plink2 \
    ...
    --pheno iid-only human_traits_fst/data/plink2_sample_popn_key.txt \
    --loop-cats PHENO1 \
    --out big_data/20210114_alfreqs_all/ "
...
bcftools query \
  --format '%CHROM\t%POS\t%ID\t%REF\t%ALT\t%INFO/AF\t%INFO/AFR_AF\t%INFO/AMR_AF\t%INFO/EAS_AF\t%INFO/EUR_AF\t%INFO/SAS_AF\n' \
  --output big_data/20210112_af_all.txt \
  vcfs/1gk_all.vcf.gz
```
...
```{r, eval = F}
# On cluster, from inside repo
## Read in data
### Test
target_files = "../big_data/20210114_alfreqs_all/by_chr/1"
data_lst <- lapply(target_files, function(target_file){
  read.table(target_file,
              header = T,
              comment.char = "")

target_dirs = list.dirs("../big_data/20210114_alfreqs_all/by_chr", recursive = F)

al_freq_lst <- lapply(target_dirs, function(target_dir){
  target_files <- list.files(target_dir, pattern = ".afreq", full.names = T)
  # read in data
  data_lst <- lapply(target_files, function(target_file){
    read.table(target_file,
               header = T,
...
df = readr::read_tsv("~/Documents/Data/20210114_hmn_fst/20210112_af_all.txt.gz",# or if on cluster "../big_data/20210112_af_all.txt", 
                col_names = c("chr", "pos", "id", "ref", "alt", "af_alt", "af_afr", "af_amr", "af_eas", "af_eur", "af_sas"),
                col_types = "cicccnnnnnn") #%>%
      # get allele frequencies for ref allele
      dplyr::mutate(af_ref = 1  - af_alt) %>%
      # re-order variables
      dplyr::select(chr, pos, id, ref, alt, af_ref, af_alt, everything()) %>%
      # filter for rows with rsIDs (some have "esv3584976"-type IDs)
      dplyr::filter(grepl("rs", df$id))

## Bin by 1% allele frequency
df$bin_100_ref = cut(df$af_ref, breaks = 100, labels = F)
df$bin_100_alt = cut(df$af_alt, breaks = 100, labels = F)

## Load other allele-frequency tables
pheno_df = read.table(here("data")
```
