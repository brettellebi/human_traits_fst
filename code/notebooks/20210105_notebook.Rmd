...
### Histograms of the global MAF distributions (i.e. in all populations together)
...
target_files = list.files("~/Documents/Data/20210114_hmn_fst/all_1",
                          pattern = "CEU|CHS|YRI", 
                          full.names = T)

data_lst = lapply(target_files, function(target_file){
  df = read.table(target_file,
                  header = T,
                  comment.char = "")
  return(df)
})
...
# fix names of populations
names(data_lst) = basename(target_files) %>% 
  stringr::str_split("\\.", simplify = T) %>% 
  subset(select = 2, drop = T)


# change name of al_freq column to match population
counter = 0
test = lapply(data_lst, function(popn_df){
  counter <<- counter + 1
  target_popn = names(data_lst)[counter]
  colnames(popn_df)[colnames(popn_df) == "ALT_FREQS"] = target_popn
  return(popn_df)
})
