...
dat_lst = lapply(dat_lst, function(pheno_df){
  ...
  dupes = unique(pheno_df$TOP_SNP[duplicated(pheno_df$TOP_SNP) | duplicated(pheno_df$TOP_SNP, fromLast = T)])
  # Select only 1 from each
  pheno_df2 = lapply(dupes, function(dupe){
    # Take the one with the lowest P-value
    min_p = min(pheno_df$P[pheno_df$TOP_SNP == dupe])
    out = pheno_df[pheno_df$TOP_SNP == dupe & pheno_df$P == min_p, ]
    # If there are still duplicated due to equal P, take the one with the largest effect size
    if (nrow(out) > 1 ){
...
length(test$TOP_SNP[duplicated(test$TOP_SNP)])
```
...
out_dir=big_data/20210114_alfreqs_all/by_chr/
popn_key=human_traits_fst/data/plink2_sample_popn_key.txt
